...
object UpdatePdfs extends Logging {
  ...
  class MyTableView(items: ObservableBuffer[UEntry]) extends TableView[UEntry](items) {
    ...
    val tcOpenboth: TableColumn[UEntry, lang.Boolean] = new TableColumn[UEntry, java.lang.Boolean] {
      text = "Open"
      cellValueFactory = { x =>
        new BooleanProperty(x, "open both", false) {
          onChange((_, _, _) => {
        ...
        atask = new javafx.concurrent.Task[Unit] {
          override def call(): Unit = {
            ...
            updfs.foreach { fileExternal =>
              if (isCancelled) {
                Helpers.runUIwait { ApplicationController.showNotification(s"Update PDFs cancelled!") }
                return
              }
              updateMessage("Compare files...")
              inTransaction {
              ...
              udone += 1
              updateProgress(udone, updfs.length)
            }
            ...
            dres match {
              case Some(ButtonType.OK) =>
                ...
                val topicUpdated = inTransaction {
                  ReftoolDB.topics.insert(new Topic(title = "Updated PDF articles " + datestamp, parent = ReftoolDB.specialTopic.id, expanded = false))
                }
            ...
            Helpers.runUIwait { ApplicationController.showNotification(s"Finished update PDFs!") }
          }
        },
        cleanup = () => {}
      ).run()
  ...
  def exportPdfs(articles: List[Article], folder: MFile, window: Window, doatend: MFile => Unit = _ => {}): Unit = {
    val res = MFile(new DirectoryChooser {
      title = "Select folder for copying documents"
      if (folder != null) if (folder.exists) initialDirectory = folder.toFile
    }.showDialog(window))
    if (res != null) {
      new MyWorker("Exporting PDFs...",
        atask = new javafx.concurrent.Task[Unit] {
          override def call(): Unit = {
            val differentFiles = new ObservableBuffer[UEntry]()
            var orphanedFiles = new ObservableBuffer[MFile]()
            updateMessage("Finding orphaned files...")
            orphanedFiles.appendAll(res.listFiles.filter(_.isFile).filter(!_.getName.startsWith(".")).toBuffer)
            updateProgress(0, articles.size)
            var adone = 0
            articles.foreach(a => {
              if (isCancelled) {
                ApplicationController.showNotification(s"Export PDFs interrupted!")
                return
              }
              updateMessage(s"Checking ${a.getFirstDocRelative}...")
              ...
              if (fileExternal.exists) {
                ...
                updateMessage(s"Copying $fileExternal...")
                info("target doesn't exist, copy: " + fileExternal)
                MFile.copy(fileReftool, fileExternal, copyAttrs = true)
              }
              adone += 1
              updateProgress(adone, articles.size)
            })

            Helpers.runUIwait { ApplicationController.showNotification(s"Copied documents to folder!") }
            ...
            if (orphanedExistingEqualFiles.nonEmpty)
              if (Helpers.runUIwait { Helpers.showTextAlert(Alert.AlertType.Warning, "Orphaned files", "Found existing files that were not exported, but which " +
            ...
            if (orphanedFiles.nonEmpty) Helpers.runUI {
            ...
            Helpers.runUIwait { ApplicationController.showNotification(s"Finished export PDFs!") }
